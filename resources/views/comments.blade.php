...
@section('content')
	...
	<table id='displayTable' class='table table-striped table-bordered hover dataTable' cellspacing='0' role='grid' width='100%'>
		<thead><tr><th>User Email</th><th>Message</th><th>Hide</th><th>User</th><th>Date</th><th>IP Address</th></tr></thead>
		<tbody></tbody>
	</table>
	</div>
	</div>
...
@section('js')
	...
	$(document).ready(function() {		
		$('#displayTable').DataTable({
			...
			'autoWidth': true,
			'ajax': {
				'url': '{{ url(config('adminlte.dashboard_url', '')) }}/data/agg_comments'
			},
			'columns': [
				{ data: 'email' },
				{ data: 'message' },
			...
			'createdRow': function ( row, data, index ) {
				$('td', row).eq(2).html('<a href="{{ url(config('adminlte.dashboard_url', '')) }}/fix/comment/'+data['DT_RowId']+'">Mark as Read</a>');
			}
		});
	} );
	</script>
